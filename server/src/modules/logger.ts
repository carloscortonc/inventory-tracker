 ...
 */
const log = (
  params: { level: "INFO" | "ERROR"; message: string } & Record<string, any>,
  stream = _stream
) => {
  ...
  const m = util
    ...
    .concat(
      " ",
      message,
      ...(Object.keys(rest).length > 0 ? [" ", util.inspect(rest)] : ""),
      "\n"
    );
  stream.write(m);
};
...
log.info = (message: string, params: Record<string, any> = {}) =>
  log({ level: "INFO", message, ...params });
log.error = (message: string, params: Record<string, any> = {}) =>
